...
def option_8_visualizations(file_path, analyzer, output):
    ...
    # --- Block B: Heatmap Visualizations ---
    for speaker in mapping.keys():
        df_speaker = df_scene1[df_scene1['speaker'] == speaker]
        if df_speaker.empty:
            continue
        df_components = df_speaker[['block_index', 'pos', 'neu', 'neg']].set_index('block_index')
        plt.figure(figsize=(6, 4))
        sns.heatmap(df_components, annot=True, cmap="YlGnBu", fmt=".1f")
        plt.title(f"Heatmap of Sentiment Components for {speaker} (Scene 1)")
        plt.xlabel("Sentiment Component (%)")
        plt.ylabel("Dialogue Block")
        plt.tight_layout()
        plt.show()
    
    # --- Block C: Scatter Plot of Compound Scores ---
    plt.figure(figsize=(10, 6))
    sns.scatterplot(data=df_scene1, x='block_index', y='compound', hue='speaker', style='speaker', s=100)
    plt.title("Scatter Plot of Compound Scores (Scene 1)")
    plt.xlabel("Dialogue Block (Scene 1)")
    ...
    # --- Block D: Sliding Window Analysis ---
    window_size = 3
    sliding_data = []
    for speaker in mapping.keys():
...
# --- New Option 9: Print All Lines from Scene 1 to output.txt ---
def option_9_print_scene1(file_path, output):
    """
    Extracts the first scene from the full text and prints all its lines to output.txt.
    This function helps verify that Scene 1 is being parsed correctly.
    """
    full_text = read_file_with_skip(file_path, skip_lines=106 if file_path=="KingLear.txt" else 0)
    scenes = re.split(r'\n(?=SCENE\s+[IVX]+\.)', full_text)
    if scenes:
        scene1 = scenes[0]
        output.write("Scene 1 Text:\n")
        output.write(scene1)
        print("Scene 1 text has been printed to output.txt.")
    else:
        msg = "No scenes found in the text.\n"
        output.write(msg)
        print(msg)
...
try:
    choice = input(
        ...
        "  9 for printing all lines from Scene 1 to output.txt\n"
        "  (or q to quit): "
    ).strip()
...
if choice in ["1", "2", "3", "4"]:
    ...
    # --- Option 9: Print All Lines from Scene 1 ---
    def option_9_print_scene1(file_path, output):
        """
        Extracts the first scene from the full text and prints all its lines to output.txt.
        This function ensures that Scene 1 is parsed correctly.
        """
        full_text = read_file_with_skip(file_path, skip_lines=106 if file_path=="KingLear.txt" else 0)
        scenes = re.split(r'\n(?=SCENE\s+[IVX]+\.)', full_text)
        if scenes:
            ...
            output.write(msg)
            print(msg)
    option_9_print_scene1(file_path, output)

else:
    msg = "Invalid choice. Please run the program again and enter a valid option (1-9 or q to quit).\n"
    print(msg)
    output.write(msg)
